...
document(<<"<?php",Rest/binary>>, Parser, Parsed) ->
    ...
    case Parser#parser.namespace =/= Parser0#parser.namespace of
        true when Parsed =/= [] ->
            Parser1 = Parser0#parser{row = Parser0#parser.namespace_row},
            throw_error(enamespace, Parser1, ?E_ERROR, Rest);
        _ -> ok
    end,
    case NParsed of
        [] ->
...
code(<<N:8,A:8,M:8,E:8,S:8,P:8,A:8,C:8,E:8,SP:8,Rest/binary>>,
     #parser{namespace_row = undefined, row = Row} = Parser, []) when
        ?OR(N,$N,$n) andalso ?OR(A,$A,$a) andalso ?OR(M,$M,$m) andalso
        ?OR(E,$E,$e) andalso ?OR(S,$S,$s) andalso ?OR(P,$P,$p) andalso
        ?OR(C,$C,$c) andalso (?IS_SPACE(SP) orelse ?IS_NEWLINE(SP)) ->
    {Rest0, Parser0} = remove_spaces(<<SP:8, Rest/binary>>, add_pos(Parser, 9)),
    {Rest1, Parser1, []} = namespace(Rest0, Parser0#parser{namespace_row = Row}, []),
    code(Rest1, Parser1, []);
code(<<N:8,A:8,M:8,E:8,S:8,P:8,A:8,C:8,E:8,SP:8,_/binary>> = Rest,
     Parser, _Parsed) when
        ...
        ?OR(E,$E,$e) andalso ?OR(S,$S,$s) andalso ?OR(P,$P,$p) andalso
        ?OR(C,$C,$c) andalso (?IS_SPACE(SP) orelse ?IS_NEWLINE(SP)) ->
    throw_error(enamespace, Parser, ?E_ERROR, Rest);
...
code(<<F:8,U:8,N:8,C:8,T:8,I:8,O:8,N:8,SP:8,Rest/binary>>, Parser, Parsed) when
    ...
    NewParser = new_line(add_pos(Parser, 8)),
    {Rest0, Parser0, #function{}=Function} =
        ephp_parser_func:st_function(Rest, NewParser, []),
    code(Rest0, copy_level(Parser, Parser0), Parsed ++ [Function]);
...
namespace(<<";", _/binary>> = Rest, Parser, Parsed) ->
    {Rest, Parser, Parsed};
namespace(<<>>, Parser, _Parsed) ->
    throw_error(eparse, Parser, <<>>);
namespace(<<A:8, Rest/binary>>, #parser{namespace = NS} = Parser, Parsed) when
        ?IS_ALPHANUM(A) orelse A =:= $_ orelse A =:= $\\ ->
    namespace(Rest, inc_pos(Parser#parser{namespace = <<NS/binary, A:8>>}), Parsed).
...
constant(<<A:8,Rest/binary>>, Parser, []) when ?IS_ALPHA(A) orelse A =:= $_ orelse A =:= $\\ ->
...
constant_known([#constant{name = <<"__NAMESPACE__">>}|Parsed], #parser{namespace = N} = Parser) ->
    [add_line(#text{text = N}, Parser)|Parsed];
