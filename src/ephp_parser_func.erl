...
funct_name(Rest, #parser{namespace = NS} = Parser, [N]) ->
    NormName = case binary:split(N, <<$\\>>, [global]) of
        [N] when NS =:= <<>> -> N;
        [N] -> <<NS/binary, $\\, N/binary>>;
        _Parts -> ephp_parser:throw_error(eparse, Parser, {unexpected, <<";">>})
    end,
    {Rest, Parser, NormName}.
